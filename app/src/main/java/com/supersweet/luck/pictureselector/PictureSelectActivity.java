...
public class PictureSelectActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCropWidth = getIntent().getIntExtra(CROP_WIDTH, 600);
        mCropHeight = getIntent().getIntExtra(CROP_HEIGHT, 600);
        ...
        for (int i = 0; i < permissions.length; i++) {
            if (grantResults[i] == PackageManager.PERMISSION_DENIED) {
                ...
                if (!ActivityCompat.shouldShowRequestPermissionRationale(this, permissions[i])) { //用户选择了"不再询问"
                    if (isToast) {
                        Toast.makeText(this, "please trun on gray", Toast.LENGTH_SHORT).show();
                        isToast = false;
                    }
                }
            }
